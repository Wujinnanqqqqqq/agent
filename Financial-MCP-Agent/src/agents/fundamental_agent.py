...
async def fundamental_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not all([api_key, base_url, model_name]):
            ...
            # 这么做的意义在于：当检测到必要的环境变量缺失时，无法继续执行后续的基本面分析流程。
            # 此时直接return，可以及时终止函数执行，并将当前的状态（包含错误信息）返回给上层调用者，
            # 让系统能够感知到本次Agent执行失败，并据此做出相应处理（如提示用户、记录日志等）。
            return {"data": current_data, "messages": current_messages, "metadata": current_metadata}
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的时间周期或其他工具组合，基于可用信息提供尽可能全面的分析。"""
            ...
            # 6. 提取分析结果
            # 这句的作用是为final_output变量设置一个默认值，防止后续提取分析结果失败时final_output未定义。
        ...
        # 这一段是最外层的异常捕获（except Exception as e），用于兜底处理整个 try 块中未被内部 except 捕获的所有异常，
        # 保证即使发生未知错误也能优雅地记录日志、写入错误信息到 current_data 和 current_metadata，
        # 并通过 execution_logger 记录 agent 执行失败，最后返回当前的 agent 状态，防止程序崩溃。
