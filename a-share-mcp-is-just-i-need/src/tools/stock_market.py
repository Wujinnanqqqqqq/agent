...
def register_stock_market_tools(app: FastMCP, active_data_source: FinancialDataSource):
    ...
    def get_historical_k_data(
        ...
        参数:
            ...
            adjust_flag: 价格/成交量调整标志。有效选项（来自Baostock）：
                           ...
                           '3': 不复权
                         默认为'3'
            fields: 可选的具体数据字段，用逗号分隔（例如：'date,code,open,high,low,close'）
                    如果为空，将使用默认字段（例如：date, code, open, high, low, close, volume, amount, pctChg）
    ...
    @app.tool()
    def get_stock_basic_info(code: str, fields: str = "") -> str:
        """
        获取给定中国A股股票的基本信息

        参数:
            code: Baostock格式的股票代码（例如：'sh.600000', 'sz.000001'）
            fields: 可选的具体数据字段，用逗号分隔（例如：'code,code_name,industry,listingDate'）
                    如果为空，返回Baostock中所有可用的基本信息列
