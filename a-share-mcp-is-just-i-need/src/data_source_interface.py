...
class FinancialDataSource(ABC): #表明这个类是一个抽象类，不能直接实例化，只作为一个规范或者合同存在
    """
    ...
    @abstractmethod
    # 标记的方法只有声明没有实现（实现就是pass）
    # 强制所有子类必须实现这些方法，否则无法实例化
    # 定义了方法签名（名称、参数、返回值），规定了"接口形式"

    # 注意这里继承ABC类中声明的方法中不需要具体实现，但在后续需要具体实现：
    # class BaostockDataSource(FinancialDataSource):
    #    def get_historical_data(self, code, date):
    #        # 这里是具体实现代码
    #        return baostock.query_history_data(code, date)
